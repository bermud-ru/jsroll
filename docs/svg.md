...
И у отдельной фигуры (`el.svg`), и у самой канвы (`canvas`) — одинаковый
набор методов для подписки/отписки/генерации событий. Работает и с
обычными DOM-событиями (`click`, `mouseover`), и с полностью
произвольными типами — `addEventListener` одинаково принимает любую
строку:
...
shape.svg.trigger('point:added', { label: 'Янв', value: 42 });
```

Событие, сгенерированное на фигуре, **всплывает** (`bubbles: true`) —
его поймает и обработчик на родительской группе, и `canvas.on(...)` на
самой канве (оба — обычные DOM-узлы). Это удобно, чтобы развязать код,
поставляющий данные (например, обработчик формы ввода), и код, который
их рисует — первый просто генерирует событие с любым именем и данными,
второй на него подписан и ничего не знает об источнике:
...
Живой пример такой развязки (форма на странице добавляет данные на
график через события) — раздел 4 в `examples/svg/index.html`.

> У растрового аналога (`jsroll.image.js`) то же самое API
> (`on`/`off`/`trigger`), но у отдельной фигуры там **нет** всплытия — это
> не DOM-узел, обычный JS-объект. Подробности — `docs/image.md`.

## Полный пример

Четыре готовых сценария (живой график, круговая диаграмма, «паутинка»
radar-chart, динамическое добавление данных из формы через
пользовательские события) — в `examples/svg/index.html`.
