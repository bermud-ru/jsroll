...
| `svg/`, `image/` | `jsroll.svg.js`/`jsroll.image.js`: живой график, круговая диаграмма, «паутинка» (radar chart), пользовательские события (`on/off/trigger`), сохранение картинки/данных в файл |
| `dataobject/` | `dataObject()` + `crud()`: локальный «сервер» в памяти вместо `xhr` — тот же интерфейс `.data`/`.get/.post/.put/.del()` |
...
18. **`grid()`-ячейки данных без явного `tabIndex` не фокусируются кликом
    мыши в реальном браузере** (обычный `<td>` по умолчанию нефокусируем —
    у него нативный `tabIndex=-1`) — значит, `focusin`/`contenteditable`
    никогда не включаются, и ячейку невозможно отредактировать. Исправлено
    в `jsroll.ui.grid.js`: `cellEvent()` теперь выставляет `tabIndex=0` при
    подключении обработчиков — как для исходных ячеек, так и для новых
    (добавленных через `row.add()`/`col.add()`). Актуально и для `grid()`,
    и для `journal()`.
19. **`crud.prototype.del()` не работает через `dataObject()`.** Метод
    подставляет во внутренний вызов `method: 'DELETE'` (полное слово, как
    настоящий HTTP-метод), а `dataObject()` внутри сравнивает
    `opt.method.toLowerCase()` именно с `case 'del':` (короткая форма,
    совпадающая с именем метода `crud`, а не с HTTP-глаголом) —
    `'DELETE'.toLowerCase()` даёт `'delete'`, что ни с одним `case` не
    совпадает, и удаление молча ничего не делает (`post`/`put`/`get`
    работают корректно — совпадение короткой/длинной формы там случайно
    сошлось). Рабочий обходной путь: вызывать функцию, которую вернул
    `dataObject()`, напрямую (минуя `crud.del()`), передав явно короткую
    форму: `api({ method: 'del', crud: rows, index: i, after: fn })`. См.
    `examples/dataobject/index.html`.
20. **`dataObject()`'s `done(...)` для `get`/`post` может получить не то,
    что ожидалось.** Внутри есть `var index = is_empty(o.index) ||
    parseInt(o.index);` — при отсутствующем `o.index` (обычная ситуация
    для `get`/`post`, где индекс не нужен) `is_empty(undefined)` возвращает
    `true` (булево), а не `null`, из-за чего код `opt.done(index !== null ?
    rows[index] : rows)` ошибочно уходит в ветку `rows[index]` (`rows[true]`
    → `undefined`) вместо ожидаемого возврата всего массива `rows`.
    Практический обход: не полагаться на аргумент `done()` для `get`/`post` —
    читать актуальные данные напрямую из `crud.data` внутри `opt.after`
    (вызывается всегда, после любой операции, независимо от этого бага).
21. **`WebAuthn`'s `rpId` по умолчанию не задаётся намеренно.** Если явно
    указать `rpId: location.hostname`, браузер сверяет его буквально с
    эффективным доменом текущего контекста — если страница открыта не как
    обычная вкладка на своём адресе (например, через встроенный
    предпросмотр/прокси/iframe), это может не совпасть, и WebAuthn ответит
    «The requested RPID did not match the origin or related origins» — не
    баг конкретного вызова, а сама механика защиты. Решение — не задавать
    `rpId` вовсе (браузер сам подставит корректный текущий домен).
22. **Нет модульной системы.** Новый код — тот же стиль: `(function (g,
